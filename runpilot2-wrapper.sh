...
function sortie() {
  ...
  if [[ -n "${CHILD}" ]] || [[ -n "$SUPERVISOR_PID" ]]; then
    log "cleanup: sending SIGTERM to supervise_pilot $SUPERVISOR_PID $CHILD"
    err "cleanup: sending SIGTERM to supervise_pilot $SUPERVISOR_PID $CHILD"
