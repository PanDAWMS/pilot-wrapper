...
function apfmon_running() {
  ...
  echo -n "running 0 ${VERSION} ${qarg} ${HARVESTER_ID}:${HARVESTER_WORKER_ID}" > /dev/udp/148.88.72.40/15778
...
function apfmon_exiting() {
  ...
  echo -n "exiting ${duration} ${VERSION} ${qarg} ${HARVESTER_ID}:${HARVESTER_WORKER_ID}" > /dev/udp/148.88.72.40/15778
...
function apfmon_fault() {
  [[ ${mute} == 'true' ]] && muted && return 0
  echo -n "fault ${duration} ${VERSION} ${qarg} ${HARVESTER_ID}:${HARVESTER_WORKER_ID}" > /dev/udp/148.88.72.40/15778
  out=$(curl -ksS --connect-timeout 10 --max-time 20 \
